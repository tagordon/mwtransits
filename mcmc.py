...
def make_data(t, log_S0, log_w0, log_Q, logsig, 
    ...
    q = np.array([1, a])
    Q = q[:, None]*q[None, :]
    diag = np.exp(2*logsig_init)*np.ones(2, len(t))
    ...
    n += transit.eval()
    return n, np.sum([n[i::2].T[0] for i in range(len(q))], axis=0)
...
def run_mcmc_2d(t, data, logS0_init, 
    ...
    with pm.Model() as model:
        sig = pm.Uniform("sig", lower=-20.0, upper=0.0, testval=logsig_init)
...
def run_mcmc_1d(t, data, logS0_init, 
    ...
    with pm.Model() as model:
        ...
        diag = np.exp(2*sig)*tt.ones((1, len(t)))
        gp = GP(kernel, t, diag, J=4, Q=Q)
